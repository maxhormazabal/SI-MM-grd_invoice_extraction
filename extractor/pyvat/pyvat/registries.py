...
class ViesRegistry(Registry):
    ...
    def check_vat_number(self, vat_number, country_code):
        ...
        request_data = (
            ...
            ' xmlns:ns0="urn:ec.europa.eu:taxud:vies:services:checkVa'
            't:types" xmlns:ns1="http://schemas.xmlsoap.org/soap/enve'
            'lope/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-insta'
            'nce" xmlns:SOAP-ENV="http://schemas.xmlsoap.org/soap/env'
            'elope/"><SOAP-ENV:Header/><ns1:Body><ns0:checkVat><ns0:c'
            "ountryCode>%s</ns0:countryCode><ns0:vatNumber>%s</ns0:va"
            "tNumber></ns0:checkVat></ns1:Body></SOAP-ENV:Envelope>"
            % (country_code, vat_number)
        )

        result.log_lines += [
            "> POST %s with payload of content type text/xml, charset UTF-8:",
            request_data,
        ]
        ...
        try:
            ...
            result.log_lines.append(
                "< Request to EU VIEW registry timed out:" " {}".format(e)
            )
            return result
        except Exception as exception:
            # Do not completely fail problematic requests.
            result.log_lines.append("< Request failed with exception: %r" % (exception))
            return result

        # Log response information.
        result.log_lines += [
            "< Response with status %d of content type %s:"
            % (response.status_code, response.headers["Content-Type"]),
            response.text,
        ]

        # Do not completely fail problematic requests.
        if response.status_code != 200 or not response.headers[
            "Content-Type"
        ].startswith("text/xml"):
            result.log_lines.append(
                "< Response is nondeterministic due to "
        ...
        if valid_text in frozenset(("true", "false")):
            ...
            result.log_lines.append(
                "< Response is nondeterministic due to "
                "invalid validity field: %r" % (valid_text)
            )

        # Parse the business name and address if possible.
        try:
            name_node = get_first_child_element(check_vat_response_node, "ns2:name")
        ...
        try:
            address_node = get_first_child_element(
                check_vat_response_node, "ns2:address"
            )
