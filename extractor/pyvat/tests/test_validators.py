...
VAT_NUMBER_CHECK_CASES = {
    ...
    "BE": [
        (
            ...
            VatNumberCheckResult(
                True,
                business_name="NV UNILEVER BELGIUM - UNILEVER BELGIQUE - "
                "UNILEVER BELGIE",
                business_address="Industrielaan 9\n1070 Anderlecht",
            ),
        ),
    ],
    "DK": [
    ...
    "IE": [
        (
            ...
            VatNumberCheckResult(
                True,
                business_name="SPLAY CONSULTING LIMITED",
                business_address="22 ADMIRAL PARK, BALDOYLE, DUBLIN 13",
            ),
        ),
    ],
    "NL": [
    ...
    def test_no_country_code(self):
        ...
        for country_code, cases in VAT_NUMBER_FORMAT_CASES.items():
            for vat_number, expected_result in cases:
                verbal_expected_result = "valid" if expected_result else "invalid"

                self.assertEqual(
                    is_vat_number_format_valid("%s%s" % (country_code, vat_number)),
                    expected_result,
                    "expected prefixed VAT number %s%s to be %s"
                    % (country_code, vat_number, verbal_expected_result),
                )
    ...
    def test_country_code(self):
        ...
        for country_code, cases in VAT_NUMBER_FORMAT_CASES.items():
            for vat_number, expected_result in cases:
                ...
                self.assertEqual(
                    is_vat_number_format_valid(vat_number, country_code),
                    expected_result,
                    "expected non-prefixed VAT number %s (%s) "
                    "to be %s" % (vat_number, country_code, verbal_expected_result),
                )
                self.assertEqual(
                    is_vat_number_format_valid(
                        "%s%s" % (country_code, vat_number), country_code
                    ),
                    expected_result,
                    "expected prefixed VAT number %s%s (%s) to be %s"
                    % (country_code, vat_number, country_code, verbal_expected_result),
                )
        ...
        for country_code, cases in VAT_NUMBER_CHECK_CASES.items():
            for vat_number, expected in cases:
                self.assert_result_equals(
                    expected, check_vat_number("%s%s" % (country_code, vat_number))
                )
    ...
    def test_dk__country_code(self):
        ...
        for country_code, cases in VAT_NUMBER_CHECK_CASES.items():
            for vat_number, expected in cases:
                ...
                self.assert_result_equals(
                    expected,
                    check_vat_number("%s%s" % (country_code, vat_number), country_code),
                )


__all__ = (
    "IsVatNumberFormatValidTestCase",
    "CheckVatNumberTestCase",
)
