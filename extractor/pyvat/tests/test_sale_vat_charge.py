    ...
    def test_get_sale_vat_charge(self):
        ...
        for seller_cc in EU_COUNTRY_CODES:
            for it in SUPPORTED_ITEM_TYPES:
                for d in [datetime.date(2014, 12, 15), datetime.date(2015, 1, 1)]:
                    for buyer_is_business in [True, False]:
                        vat_charge = get_sale_vat_charge(
                            d,
                            it,
                            Party(
                                country_code=seller_cc, is_business=buyer_is_business
                            ),
                        ...
                        self.assertEqual(
                            vat_charge.rate, EXPECTED_VAT_RATES[seller_cc][it]
                        )
            ...
            for buyer_cc in EU_COUNTRY_CODES:
                ...
                for it in SUPPORTED_ITEM_TYPES:
                    for d in [datetime.date(2014, 12, 15), datetime.date(2015, 1, 1)]:
                        ...
                        self.assertEqual(
                            vat_charge.action, VatChargeAction.reverse_charge
                        )
                ...
                for it in SUPPORTED_ITEM_TYPES:
                    for d in [datetime.date(2014, 12, 15), datetime.date(2015, 1, 1)]:
                        ...
                        self.assertEqual(
                            ...
                            if d >= datetime.date(2015, 1, 1)
                            else EXPECTED_VAT_RATES[seller_cc][it],
                        )
                        self.assertEqual(
                            vat_charge.country_code,
                            buyer_cc if d >= datetime.date(2015, 1, 1) else seller_cc,
                        )
                    ...
                    for d in [datetime.date(2014, 12, 15), datetime.date(2015, 1, 1)]:
                        for buyer_is_business in [True, False]:
                            vat_charge = get_sale_vat_charge(
                                ...
                                it,
                                Party(
                                    country_code=buyer_cc, is_business=buyer_is_business
                                ),
                                Party(country_code=seller_cc, is_business=True),
                            )
                            self.assertEqual(
                                vat_charge.action, VatChargeAction.no_charge
                            )
                            self.assertEqual(vat_charge.rate, Decimal(0))
